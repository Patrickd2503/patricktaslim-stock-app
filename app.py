...
# --- 3. FUNGSI EXPORT (PENARIKAN DATA LENGKAP) ---
def generate_combined_report(report_choice, df_short, df_all, df_c):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        if report_choice == "MFI & Big Volume":
            ...
            # Fitur: Export data harian lengkap (Persentase dan Harga)
            df_pct_raw = (df_c.pct_change() * 100)
            df_pct_raw.to_excel(writer, index=True, sheet_name='1. Persentase Harian')
            df_c.to_excel(writer, index=True, sheet_name='2. Harga Nominal')
...
if btn_analisa:
    with st.spinner('Menghitung data...'):
        ...
        if not df_c.empty:
            ...
            with tab2:
                # Menampilkan persentase harian dalam bentuk Transpose (Ticker sebagai baris, Tanggal sebagai kolom)
            ...
            with tab3:
                # Menampilkan harga nominal harian
