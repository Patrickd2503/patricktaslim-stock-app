...
# --- 1. FITUR CACHE DATA (Integrasi Source 1, 2, 3) ---
@st.cache_data(ttl=3600)
def fetch_yf_all_data(tickers, start_date, end_date):
    # Buffer data 12 bulan untuk analisa histori ARA dan MFI [cite: 108]
    ...
    try:
        ...
        if df.empty:
            return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), pd.DataFrame() [cite: 82]
        
        if isinstance(df.columns, pd.MultiIndex):
            return df['Close'], df['Volume'], df['High'], df['Low'] [cite: 82]
        else:
            return df[['Close']], df[['Volume']], df[['High']], df[['Low']] [cite: 82]
...
def load_data_auto():
    # Mencari file database lokal sesuai referensi [cite: 110, 134]
...
def generate_combined_report(report_choice, df_short, df_all, df_c):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        elif report_choice == "Split View (Persentase & Harga)":
            # Format Source 5 Jan [cite: 139, 153]
            df_pct_full = (df_c.pct_change() * 100).applymap(lambda x: f"{x:.1f}%" if pd.notnull(x) else "0.0%")
...
def run_integrated_analysis(df_c, df_v, df_h, df_l, df_ref):
    ...
    for col in df_c.columns:
        ...
        if len(c) < 30: continue [cite: 95]
        
        # Logika MFI (Source 1) [cite: 96]
        tp = (h + l + c) / 3
        mf = tp * v
        ...
        # Logika Volume Control (Source 3) [cite: 141]
        ...
        # Penentuan Shortlist (Gabungan kriteria) [cite: 98, 143]
        if c.iloc[-1] > ma20 and last_mfi < 65 and v_ratio >= 1.2:
            shortlist_keys.append(ticker)
        
        results.append({
            'Kode Saham': ticker, [cite: 99]
            'Last Price': int(c.iloc[-1]),
            'MFI (14D)': round(last_mfi, 2),
            'Vol Control (%)': f"{vol_control:.1f}%", [cite: 145]
            'Max Gain (12M)': f"{max_gain_12m:.1f}%", [cite: 121]
            'Freq ARA': int(count_ara),
            'Above MA20': is_above_ma20,
            'Vol Ratio': round(v_ratio, 2) [cite: 100]
        })
...
if btn_analisa:
    with st.spinner('Memproses analisa...'):
        ...
        if not df_c.empty:
            ...
            st.sidebar.markdown("---")
            st.sidebar.subheader("📥 Penarikan Report") [cite: 107, 129]
            ...
            st.error("Data tidak ditemukan.") [cite: 160]
else:
