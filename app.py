...
def fetch_yf_all_data(tickers, start_date, end_date):
    # Mengambil Close dan Volume sekaligus
...
def get_signals(df_c, df_v):
    ...
    for col in df_c.columns:
        ...
        # Cek volatilitas harga 5 hari terakhir
        change_5d = (c.iloc[-1] - c.iloc[-5]) / c.iloc[-5]
        # Cek volume vs rata-rata
        v_sma5 = v.rolling(5).mean().iloc[-1]
        v_last = v.iloc[-1]
        
        # LOGIKA: Harga tenang (abs < 2%), tapi volume naik (>20% dari rata-rata)
...
if df_emiten is not None:
    ...
    today = date.today()
    start_d = st.sidebar.date_input("Mulai", today - timedelta(days=20)) # Rentang lebih panjang untuk analisa
            ...
            if not df_c_raw.empty:
                ...
                if not df_f_c.empty:
                    ...
                    def prepare_final(df_data, is_pct=True):
                        ...
                        return pd.merge(merged, df_sig, on='Kode Saham', how='left')
                    ...
                    if tipe in ["Perubahan (%)", "Split View (Keduanya)"]:
                        ...
                        st.dataframe(df_pct.style.applymap(style_target, subset=df_pct.columns[2:-1]), use_container_width=True)
                    ...
                    if tipe in ["Harga Penutupan (IDR)", "Split View (Keduanya)"]:
                        ...
                        st.dataframe(df_prc.style.applymap(style_target, subset=df_prc.columns[2:-1]), use_container_width=True)
                else:
